...
export class HeimDist {
    ...
    protected async getHeimInfo(): Promise<HeimInfo> {
        ...
        if(this.platform === 'win32') {
            ...
            core.debug(`Platform ${this.platform} does not have an offical build installing linux version`);
            platform = 'linux';
        }

        let version: semver.SemVer;
        let inputVersion = this.version;
        if(inputVersion === 'current' || inputVersion === 'latest' || inputVersion.startsWith('v')) {
            const resp = await this.httpClient.getJson<IHeimVersions>(`https://cloud.heim.dev/heim/release?version=${inputVersion}`);
        ...
        let file: string = platform === 'windows'
            ? `${filename}.zip`
            : `${filename}.tar.gz`;
    ...
    protected async downloadHeim(info: HeimInfo): Promise<string> {
        ...
        const toolPath = await this.extractArchive(
            downloadPath, 
            info
        );
        core.info('Done');
