...
describe('heim-setup', () => {
    ...
    beforeEach(() => {
        ...
        platformSpy.mockImplementation(() => { return os['platform']; });
    ...
    describe('main', () => {
        ...
        it('Complete version in cache', async () => {
            ...
            expect(setOutputSpy).toHaveBeenNthCalledWith(1, 'heimHome', path.normalize(`${toolPath}heim`));
            expect(setOutputSpy).toHaveBeenNthCalledWith(2, 'bin', path.normalize(`${toolPath}heim/bin`));
            expect(setOutputSpy).toHaveBeenNthCalledWith(3, 'runtime', path.normalize(`${toolPath}heim/bin/heim-runtime`));
            expect(setOutputSpy).toHaveBeenNthCalledWith(4, 'cli', path.normalize(`${toolPath}heim/bin/heim`));
        })
        it('Complete version not in cache', async () => {
            ...
            expect(setOutputSpy).toHaveBeenNthCalledWith(1, 'heimHome', path.normalize(`${cachePath}heim`));
            expect(setOutputSpy).toHaveBeenNthCalledWith(2, 'bin', path.normalize(`${cachePath}heim/bin`));
            expect(setOutputSpy).toHaveBeenNthCalledWith(3, 'runtime', path.normalize(`${cachePath}heim/bin/heim-runtime`));
            expect(setOutputSpy).toHaveBeenNthCalledWith(4, 'cli', path.normalize(`${cachePath}heim/bin/heim`));
        })
        ...
        it('Invalid version', async () => {
            ...
            expect(setFailedSpy).toHaveBeenCalledWith(`Unabled to find a version matching ${version}`);
        })
    });
    ...
    describe('HeimDist', () => {
        it('Unsupported archeitecture', async () => {
            ...
            await expect(heimDist.setupHeim())
                    .rejects
                    .toThrow(`Unsupported architecture '${os.arch}'`);
        })
        ...
        it('Unable to find a version matching', async () => {
            ...
            await expect(heimDist.setupHeim())
                    .rejects
                    .toThrow(`Unabled to find a version matching ${version}`);
        })
        ...
        it('Failed to download heim', async () => {
            ...
            await expect(heimDist.setupHeim())
                    .rejects
                    .toThrow(`Unexpected HTTP response: 404`);
        });
        ...
        it.each([
        ...
        ])(`find tool in cache, os: %s, arch: %s, version: %s`, async (
            platform,
            arch,
            version,
            expArch,
            ...
            if(version !== expVersion) {
                expect(getJsonSpy).toHaveBeenCalledWith(`https://cloud.heim.dev/heim/release?version=${version}`)
            }

            expect(infoSpy).toHaveBeenNthCalledWith(1, `Found in cache @ ${toolPath}`);
        ...
        it.each([
        ...
        ])('tool not in cache, os: %s, arch: %s, version: %s', async (
            platform,
            arch,
            version,
            expPlat,
            ...
            const cachePath = path.normalize(`/cache/${expVersion}/${expArch}/heim/`)
            cacheDirSpy.mockImplementation(async () => cachePath);
            ...
            expect(heimPaths).toStrictEqual(<HeimPaths> {
                ...
                heimRuntime: path.normalize(`${cachePath}heim/bin/heim-runtime${fileExt}`),
